...
10. **Bug en `deposit()`: `MoneyDeposited.previousAmount` guardaba el balance equivocado.** `this.balance = this.balance.add(amount)` se ejecutaba *antes* de construir el evento, así que `previousAmount` en realidad quedaba con el balance posterior al depósito, no el anterior (asimetría con `withdraw()`, que sí lo hacía bien). Detectado escribiendo el test `depositShouldRaiseMoneyDepositedEventWithPreviousAndDepositedAmounts` en `WalletTest`, que afirmaba el comportamiento correcto y falló contra el código real. Se corrigió invirtiendo el orden, igual que en `withdraw`: calcular `newBalance`, construir el evento con el balance viejo, reasignar al final.
11. **Suite de tests del dominio y de los use cases.** Se agregaron:
    - Unit tests puros (sin Spring): `WalletTest`, `MoneyTest`, `WalletIdTest`, `OwnerIdTest` (47 tests, todos en `src/test/java/.../domain/`).
    - Integration tests (`@SpringBootTest` + `@Transactional`, contra el Postgres real, con `DomainEventPublisher` reemplazado por `@MockitoBean` para no depender de Kafka): uno por cada use case (`CreateWalletUseCaseIntegrationTest`, `GetWalletUseCaseIntegrationTest`, `DepositMoneyUseCaseIntegrationTest`, `WithdrawMoneyUseCaseIntegrationTest`, `FreezeWalletUseCaseIntegrationTest`, `ActivateWalletUseCaseIntegrationTest`), en `src/test/java/.../application/usecase/`.

## Pendiente

1. **`spring-boot-starter-validation` está en el `pom.xml` pero no se usa** en ningún DTO.
2. **`InvalidWalletException` sigue siendo un único tipo genérico** para todos los errores de negocio (wallet no encontrada, ya congelada, ya activa, saldo insuficiente...). Por eso `WalletExceptionHandler` mapea *todo* a `409 Conflict`, incluso "wallet no encontrada", que semánticamente debería ser `404 Not Found`. Para distinguirlo bien haría falta una jerarquía de excepciones de dominio (ej. `WalletNotFoundException` separada).
...
Ya no hay ningún bloqueante para correr el servicio, ni bugs funcionales conocidos, y el dominio y los use cases ya tienen cobertura de tests. Lo que queda es deuda de calidad (puntos 1-2 de "Pendiente"). Buen candidato para la próxima sesión: la jerarquía de excepciones de dominio (punto 2), que además mejora el `WalletExceptionHandler` ya existente.
